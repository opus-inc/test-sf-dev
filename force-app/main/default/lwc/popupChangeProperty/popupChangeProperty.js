...
import getBlocosByEmpreendimento from '@salesforce/apex/SelectPropertyController.getBlocosByEmpreendimento';
...
import changeUnidadeProposta from '@salesforce/apex/PropostaCadastroController.changeUnidadeProposta';
...
export default class SelectPropertyInformation extends NavigationMixin(LightningElement) {
    ...
    @track empreendimentoId;
    @track propostaId;
    @track disableEmpreendimentoSelect;

    @track infoSelectedResumo;

    @api viewModal(empreendimentoId, propostaId) {
    ...
    connectedCallback() {
        this.isLoading = true;
        getBlocosByEmpreendimento({ empreendimentoId: this.empreendimentoId, corretorId: this.corretorId }).then(result => {
            if (result.length > 0) {
                this.empreendimentos = result
                    .filter(i => i.Empreendimento__c === this.empreendimentoId)
                    .filter(i => i.Tabelas_de_Vendas__r).sort((a,b) => {
                ...
                if(result.filter(i => i.Empreendimento__c === this.empreendimentoId).length === 1) {
                    // this.empreendimentoSelectedId = this.empreendimentos[0].Id;
                    this.disableEmpreendimentoSelect = true;
                    this.handleChangeEmpreendimento({
                        target: { 
                            options: this.empreendimentos
                        },
                        detail: {
                            value: this.empreendimentos[0].value,
                        }
                    })
                }
            }
    ...
    handleChangeEmpreendimento(event) {
        ...
        console.log(this.limitarPorTipologia);
        if(this.limitarPorTipologia) {
    ...
    handleChangeGaragemCombo(event) {
        if (event.detail.value !== 'Selecione garagem'
            ...
            var label = event.target.options.find(opt => opt.value === event.detail.value).label;
            this.garagensComboSelected = [{ Id: event.detail.value, label }];
            this.garagensSelectedResumo = [...this.garagensComboSelected, ...this.garagensSelected].map(element => element.label.split('|')[0]).join(",");
    ...
    handleClickSimular(event) {
        ...
        changeUnidadeProposta(body).then(result => {
    ...
    calculateProposal() {
        ...
        this.totalProposta = (this.totalValorGaragens + this.totalValorUnidades + this.totalItensAdicionais);
